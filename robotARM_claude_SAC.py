...
class Config:
    ...
    ALGORITHM = "SAC_HER"  # Change to "PPO" for multi-core PPO training
    ...
    # SAC-specific
    USE_FIXED_ENTROPY = True  # Set to True for fixed entropy coef (more stable)
    FIXED_ENTROPY_COEF = 0.2  # Higher = more exploration (0.1-0.5 for robotics)

    # Environment
    ENV_NAME = "PandaPickAndPlace-v3"
    REWARD_TYPE = "sparse"  # HER works MUCH better with sparse rewards!
...
def train_sac_her():
    ...
    print(f"{'='*60}")
    print(f"Environment: Single (HER works best with 1 env)")
    print(f"Device: {Config.DEVICE}")
    print(f"Total Steps: {Config.TOTAL_TIMESTEPS:,}\n")
    ...
    # Use SINGLE environment for optimal HER performance
    ...
    # Choose between fixed or auto entropy
    if Config.USE_FIXED_ENTROPY:
        ent_coef = Config.FIXED_ENTROPY_COEF
        print(f"[INFO] Using FIXED entropy coefficient: {ent_coef}")
    else:
        ent_coef = 'auto_0.1'
        print(f"[INFO] Using AUTO entropy tuning (starting from 0.1)")
    ...
    model = SAC(
        ...
        replay_buffer_kwargs=dict(
            n_sampled_goal=8,  # Increased from 4 for better learning
            goal_selection_strategy="future",
            #max_episode_length=50,  # Explicit episode length
        ),
        learning_rate=1e-3,
        buffer_size=1_000_000,
        learning_starts=1000,  # Wait for 1000 steps before training
        batch_size=256,
        gamma=0.95,
        tau=0.05,
        ent_coef=ent_coef,  # Entropy coefficient (fixed or 'auto')
        train_freq=4,  # Train every step (single env)
        gradient_steps=4,  # 1 gradient step per env step
    ...
    # Setup callbacks (single environment)
    eval_callback = EnhancedEvalCallback(
        eval_env,
        eval_freq=Config.EVAL_FREQ,
        n_eval_episodes=Config.N_EVAL_EPISODES,
