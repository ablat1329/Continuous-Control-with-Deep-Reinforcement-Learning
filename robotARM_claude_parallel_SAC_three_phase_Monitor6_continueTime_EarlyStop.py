...
class Config:
    ...
    PHASES = [
        {"timesteps": 1000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 1e-3},
        {"timesteps": 2000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 8e-4},
        {"timesteps": 3000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 3e-4},
    ]
    ...
    MODEL_DIR = "models_continueTime3"
    SAVE_DIR = "renders"
    ...
    # Early stopping
    EARLY_STOP_SUCCESS_RATE = 0.99  # Stop if 95% success rate achieved
    EARLY_STOP_PATIENCE = 4  # Must maintain for 4 consecutive evaluations (4 x 25k = 100k steps)
        ...
        if steps_since_last_eval >= self.eval_freq:
            ...
            if success_rate >= self.early_stop_success_rate:
                ...
                if self.success_streak >= self.early_stop_patience:
                    ...
                    final_path = os.path.join(Config.MODEL_DIR, "sac_her_final_early_stop")
...
def train_resume_three_phase():
    ...
    # Save final model
    if model is None:
        print("[WARN] Training complete across all phases. No model to be saved. Skipping...")
    else:
