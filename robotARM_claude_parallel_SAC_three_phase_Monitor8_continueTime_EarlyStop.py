...
class Config:
    ...
    PHASES = [
        {"timesteps": 2_000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 1e-3},
        {"timesteps": 3_000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 8e-4},
        {"timesteps": 4_000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 3e-4},
    ]
    ...
    MODEL_DIR = "models_continueTime6"
    SAVE_DIR = "renders"
...
_interrupted = False
_model_saved = False  # Flag to prevent double-saving
_current_model = None
_current_env = None
...
def cleanup_on_exit():
    ...
    global _current_model, _current_env, _current_eval_env, _current_phase_idx, _model_saved

    if _current_model is not None and not _model_saved:
        print("\n" + "="*60)
        print("🛑 CLEANUP: Saving interrupted model...")
        print("="*60)

        # Use model timesteps + readable timestamp instead of Unix time
        timesteps = _current_model.num_timesteps
        timestamp_str = time.strftime("%Y%m%d_%H%M%S")
        interrupted_path = os.path.join(Config.MODEL_DIR, f"SystemEXITED_phase_{_current_phase_idx}_{timesteps}_steps_{timestamp_str}")
        try:
            _current_model.save(interrupted_path)
            _model_saved = True  # Mark as saved
...
def signal_handler(signum, frame):
    ...
    print("[INFO] Model will be saved (please wait)...")
    print("="*60)
...
def train_resume_three_phase():
    ...
    for idx, phase in enumerate(Config.PHASES):
        ...
        try:
            ...
            if not _model_saved:  # Only save if not already saved by atexit
                print("\n[INFO] KeyboardInterrupt detected - saving model and exiting gracefully.")
                # Use timesteps + readable timestamp
                timesteps = model.num_timesteps
                timestamp_str = time.strftime("%Y%m%d_%H%M%S")
                interrupted_path = os.path.join(Config.MODEL_DIR, f"interrupted_phase_{idx}_{timesteps}_steps_{timestamp_str}")
    ...
    # Save final model
    if model is None:
        print("[ERROR] Model not found. Could not save final model.")
        sys.exit(1)
    else:
