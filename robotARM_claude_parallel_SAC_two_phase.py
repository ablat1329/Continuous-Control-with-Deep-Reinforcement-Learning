...
class Config:
    ...
    # Phase 0: strong HER + more exploration
    # Phase 1: calmer HER + fine-tune
    PHASES = [
        {"timesteps": 500_000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 1e-3},
        {"timesteps": 4_500_000, "n_sampled_goal": 4, "ent_coef": "auto", "learning_rate": 1e-3},
    ]
    ...
    EVAL_FREQ = 50_000
    N_EVAL_EPISODES = 5
    CHECKPOINT_FREQ = 200_000  # saves checkpoints every this many timesteps
...
class EvalAndSaveCallback(BaseCallback):
    def __init__(self, eval_env, eval_freq=Config.EVAL_FREQ, n_eval_episodes=Config.N_EVAL_EPISODES,
        ...
        if not os.path.exists(Config.CSV_LOG):
            with open(Config.CSV_LOG, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["timesteps", "mean_reward", "std_reward", "success_rate", "timestamp"])
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.eval_freq == 0:
            ...
            print(f"\n[EVAL] Step {ts:,}: mean_reward={mean_reward:.3f} ± {std_reward:.3f}, success_rate={success_rate*100:.1f}%")
            ...
            with open(Config.CSV_LOG, "a", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([ts, float(mean_reward), float(std_reward), float(success_rate), time.time()])
...
def debug_rollouts(model, env, n_episodes=3, save_dir="debug_renders"):
    ...
    for ep in range(n_episodes):
        ...
        while True:
            ...
            print(f"step {step}: action min {amin:.4f}, max {amax:.4f}, mean {amean:.4f}")
            obs, reward, terminated, truncated, info = env.step(action)
            if step % 5 == 0:
                frames.append(env.render())
            if info:
                # print common helpful keys
                print("info keys:", list(info.keys()))
                for k in ["is_success", "grasp_success", "gripper_open", "object_pos", "achieved_goal", "desired_goal", "distance"]:
                    if k in info:
                        print(f"  {k}: {info[k]}")
            step += 1
            if terminated or truncated or step > 500:
                if frames:
                    gif_path = os.path.join(save_dir, f"debug_ep_{ep+1}.gif")
                    imageio.mimsave(gif_path, frames, fps=10)
...
def train_resume_two_phase():
    # single envs for HER
    env = gym.make(Config.ENV_NAME, reward_type=Config.REWARD_TYPE, render_mode="rgb_array")
    ...
    for idx, phase in enumerate(Config.PHASES):
        ...
        # Build new model for this phase
        if model is None:
            ...
            # Subsequent phase - create new model and transfer weights
            new_model = build_sac_with_her(env, n_sampled_goal=phase["n_sampled_goal"],
                                          ent_coef=phase["ent_coef"], learning_rate=phase["learning_rate"])
            # Transfer policy weights from previous phase
            try:
                new_model.policy.load_state_dict(model.policy.state_dict())
                print("[INFO] Transferred policy weights to new model for this phase.")
            except Exception as e:
                print(f"[WARN] Could not transfer weights: {e}")
            model = new_model
        ...
        try:
            model.learn(total_timesteps=phase_timesteps, callback=[eval_cb, checkpoint_cb], progress_bar=True)
